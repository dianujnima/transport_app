...
<div class="row">
    <div class="col-lg-12">
        <div class="card-box">
            <h4 class="header-title m-t-0">Edit Provider Schedule</h4>
            <p class="text-muted font-14 m-b-20">
                Here you can edit provider schedule.
            </p>

            <form action="{{ route('admin.schedules.save_edit') }}" class="ajaxForm" method="post" enctype="multipart/form-data" novalidate>
                @csrf
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="vehicle_type" parsley-trigger="change" class="form-control" id="vehicle_type">
                        ...
                        <option @if($schedule->type === 'standard') selected @endif value="standard">Standard</option>
                        <option @if($schedule->type === 'luxury') selected @endif value="luxury">Luxury</option>
                        <option @if($schedule->type === 'super_luxury') selected @endif value="super_luxury">Super Luxury</option>
                    </select>
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <select onchange="change_vehicle_type(this)" name="category" parsley-trigger="change" class="form-control" id="category">
                        ...
                        @foreach($categories as $category)
                            <option @if($schedule->category_id === $category->id) selected @endif data-type="{{($category->seat_types)}}" value="{{$category->id}}">{{$category->name}}</option>
                        @endforeach
                    </select>
                </div>

                <div class="form-group mb-3 seats_settings border-bottom border-dark pb-3">
                    <div id="seats_settings" class="row">
                        @foreach($schedule->seats as $seat)
                            <div class="col">
                                <div class="d-flex flex-wrap align-items-center">
                                    <input checked onchange="add_seat_validation(this)" type="checkbox" name="seats[{{$seat->seat_type}}][check]" id="type_checkbox_{{$seat->seat_type}}" data-parsley-multiple="type_checkbox" value="{{$seat->seat_type}}" data-parsley-mincheck="1" data-parsley-required data-parsley-error-message="Please choose at least 1 type of seats" /> 
                                    <label for="type_checkbox_{{$seat->seat_type}}" class="seat_type_label ml-2">{{ucwords($seat->seat_type)}} Seats</label>
                                </div>
                                <p class="m-0">Per Seat Price (in PKR)</p>
                                <input type="number" step="any" name="seats[{{$seat->seat_type}}][price]" value="{{$seat->cost}}" placeholder="Enter Seat Price" class="form-control seats_inputs mb-2" data-parsley-required>
                                <p class="m-0">Total No Of Seats</p>
                                <input type="number" name="seats[{{$seat->seat_type}}][qty]" value="{{$seat->total_seats}}" placeholder="Enter Total Seats" class="form-control seats_inputs" data-parsley-required>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <select data-toggle="select2" name="route_from" parsley-trigger="change" class="form-control" id="route_from">
                        ...
                        @foreach($cities as $city)
                            <option @if($schedule->route_from === $city->name) selected @endif value="{{$city->name}}">{{$city->name}} - ({{$city->province}})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <select data-toggle="select2" name="route_to" parsley-trigger="change" class="form-control" id="route_to">
                        ...
                        @foreach($cities as $city)
                            <option @if($schedule->route_to === $city->name) selected @endif value="{{$city->name}}">{{$city->name}} - ({{$city->province}})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" name="schedule_date" parsley-trigger="change" data-parsley-required placeholder="Enter Schedule Date" class="form-control human_datepicker" id="schedule_date" value="{{ $schedule->date ?? '' }}">
                </div>

                <div class="form-group mb-3">
                    <label for="arrival_time">Departure Time<span class="text-danger">*</span></label>
                    <input type="text" name="arrival_time" parsley-trigger="change" data-parsley-required placeholder="Enter Vehicle Arrival Time" class="form-control human_timepicker" id="arrival_time" value="{{ $schedule->arrival_time ?? '' }}">
                </div>

                <div class="form-group mb-3">
                    <label for="destination_time">Destination/Arrival Time<span class="text-danger">*</span></label>
                    <input type="text" name="destination_time" parsley-trigger="change" data-parsley-required placeholder="Enter Vehicle Destination Time" class="form-control human_timepicker" id="destination_time" value="{{ $schedule->destination_time ?? '' }}">
                </div>
                ...
                @if($schedule->image)
                    @php
                        $image = check_file($schedule->image, 'vehicle');
                    @endphp
                    <div class="form-group my-3">
                        <a href="{{$image}}" class="mr-2" download>
                            <img src="{{$image}}" alt="Vehicle Image" class="img-fluid fit-image avatar-xl rounded-circle">
                        </a>
                    </div>
                @endif
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea rows="4" placeholder="Enter Schedule Description" name="desc" parsley-trigger="change" minlength="10" class="form-control" id="desc">{{ $schedule->desc ?? '' }}</textarea>
                </div>

                <div class="form-group mb-3 text-right">
                    <input type="hidden" name="schedule_id" value="{{$schedule->hashid}}">
